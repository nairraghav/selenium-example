class AlertBoxPage:
    def __init__(self, driver):
        ...
        self.explanation_text = "There are three main JavaScript methods which show alert dialogs: alert, confirm and prompt. This page has examples of each."
        self.alert_box_descriptions_css = "div.page-body > p"
        self.alert_box_description_text = "The following button will display an alert when clicked."
        self.alert_box_button_id = "alertexamples"
        self.alert_box_button_text = "Show alert box"
        self.alert_box_text = "I am an alert box!"
        self.confirm_box_description_text = "The following button will display a confirm dialog when clicked."
        ...
        self.prompt_box_description_text = "The following button will display a prompt dialog when clicked."
    ...
    def validate_text_on_page(self):
        ...
        for element_id, text in (
            ...
            found_element = self.driver.find_element_by_id(element_id)
            assert found_element.get_attribute("value") == text, f"Actual: {found_element.get_attribute('value')}\t\tExpected: {text}"

        descriptions = self.driver.find_elements_by_css_selector(self.alert_box_descriptions_css)
    ...
    def interact_with_confirm_box(self):
        ...
        alert = self.driver.switch_to.alert
        alert.accept()
        assert self.driver.find_element_by_id(self.confirm_boolean_text_id).text == self.confirm_boolean_confirm_text
        assert self.driver.find_element_by_id(self.confirm_text_id).text == self.confirm_text

        self.driver.find_element_by_id(self.confirm_box_button_id).click()
        alert = self.driver.switch_to.alert
        alert.dismiss()
        assert self.driver.find_element_by_id(self.confirm_boolean_text_id).text == self.confirm_boolean_cancel_text
        assert self.driver.find_element_by_id(self.confirm_text_id).text == self.cancel_text
    ...
    def interact_with_prompt_box(self):
        ...
        alert.accept()
        assert self.driver.find_element_by_id(self.prompt_value_text_id).text == alert_text
        assert self.driver.find_element_by_id(self.prompt_text_id).text == f"{self.prompt_text_prefix}{alert_text}"

        self.driver.find_element_by_id(self.prompt_box_button_id).click()
        alert = self.driver.switch_to.alert
        alert.dismiss()
        assert self.driver.find_element_by_id(self.prompt_text_id).text == self.prompt_cancel_text
